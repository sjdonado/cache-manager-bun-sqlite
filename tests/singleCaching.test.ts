...
import createSqliteStore, { SqliteStore } from '../index.js';

describe('SingleCaching', () => {
  let cache: Cache<SqliteStore>;

  beforeEach(async () => {
    const store = await createSqliteStore({ path: getRandomPath() });
    cache = (await cacheManager.caching(store)) as Cache<SqliteStore>;
  });

  describe('Initialization', () => {
    it('should open store via options', async () => {
      const store = await createSqliteStore({ name: 'fool', path: '/tmp/cache.db' });
      const cache = (await cacheManager.caching(store)) as Cache<SqliteStore>;
  ...
  describe('Caching Operations', () => {
    ...
    it('should handle various TTL combinations in set', async () => {
      ...
      await cache.set(key3, value3, 200);
